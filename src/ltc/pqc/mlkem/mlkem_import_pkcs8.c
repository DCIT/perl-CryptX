...
static int s_mlkem_import_pkcs8_asn1(ltc_asn1_list *alg_id, ltc_asn1_list *priv_key,
                            mlkem_key *key)
{
   ...
   if (err == CRYPT_OK) {
      err = mlkem_make_key_from_seed(alg, seed_buf, seed_custom[0].size, key);
   ...
   /* raw_key is byte-identical to what the seed expands to, so seed_key already is the
      key we would rebuild, take it over instead of expanding a second time */
   *key = seed_key;
   XMEMSET(&seed_key, 0, sizeof(seed_key));
   err = CRYPT_OK;
...
/**
  Import a ML-KEM private key from its PKCS#8 elements
...
int mlkem_import_pkcs8_asn1(ltc_asn1_list *alg_id, ltc_asn1_list *priv_key, mlkem_key *key)
{
   ...
   if ((err = s_mlkem_import_pkcs8_asn1(alg_id, priv_key, key)) != CRYPT_OK) {
      return err;
   }
