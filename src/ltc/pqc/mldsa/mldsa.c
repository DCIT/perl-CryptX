                                       ...
                                       int k, int l)
{
   ...
   for (i = 0; i < k; ++i) {
      for (j = 0; j < l; ++j) {
         if ((err = s_poly_uniform(&a, rho, (unsigned int)((i << 8) + j))) != CRYPT_OK) return err;
                           ...
                           const mldsa_params *p)
{
   ...
   mldsa_polyvecl s1, y, z;
   mldsa_polyveck t0, s2, w1, w0, h;
   mldsa_poly cp, *mat;
   hash_state state;
   int err;

   /* the matrix is reused by every rejection iteration, so it is expanded once and
      kept on the heap, k*l polynomials are far too much for the stack */
   mat = XMALLOC((unsigned long)p->k * p->l * sizeof(*mat));
   if (mat == NULL) return CRYPT_MEM;
   ...
   if ((err = s_unpack_sk(rho, tr, key, &t0, &s1, &s2, sk, p)) != CRYPT_OK) goto LBL_ERR;
                             ...
                             const mldsa_params *p)
{
   ...
   /* msg is allowed to be NULL for an empty message, the hashes reject a NULL pointer */
   if (msglen > 0) {
   ...
   s_polyvecl_ntt(&z, p->l);
   if ((err = s_polyvec_matrix_expand_mul(&w1, rho, &z, p->k, p->l)) != CRYPT_OK) { XFREE(buf); return err; }
...
int mldsa_import_raw(const unsigned char *in, unsigned long inlen, int which, int alg, mldsa_key *key)
{
   ...
   if (which == PK_PUBLIC) {
         ...
         then compute t = A*s1 + s2, t1 = power2round(t). */
      {
         ...
         err = s_unpack_sk(rho, tr, kk, &t0, &s1, &s2, in, &p);
         ...
         if (err == CRYPT_OK) {
            ...
            s_polyveck_power2round(&t1, &t0, &t1, p.k);
            encoded_t0 = key->sk + p.sk_bytes - (unsigned long)p.k * MLDSA_POLYT0_PACKEDBYTES;
            t0_mismatch = 0;
            for (i = 0; i < (unsigned)p.k; ++i) {
         ...
         if (err == CRYPT_OK) {
            ...
            if (key->pk == NULL) {
               ...
               s_pack_pk(key->pk, rho, &t1, p.k);
               if ((err = s_shake256(computed_tr, MLDSA_TRBYTES, key->pk, p.pk_bytes)) == CRYPT_OK) {
