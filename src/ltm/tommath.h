...
#if defined(MP_16BIT)
typedef mp_u16             mp_digit;
#   define MP_DIGIT_BIT 15
#elif ((defined (MP_64BIT)) && !(defined(MP_31BIT)) )
typedef mp_u64 mp_digit;
#   define MP_DIGIT_BIT 60
#else
typedef mp_u32             mp_digit;
#   ifdef MP_31BIT
/*
...
/* get integer, set integer and init with integer (mp_i32) */
mp_i32 mp_get_i32(const mp_int *a) MP_WUR;
void mp_set_i32(mp_int *a, mp_i32 b);
mp_err mp_init_i32(mp_int *a, mp_i32 b) MP_WUR;

/* get integer, set integer and init with integer, behaves like two complement for negative numbers (mp_u32) */
#define mp_get_u32(a) ((mp_u32)mp_get_i32(a))
void mp_set_u32(mp_int *a, mp_u32 b);
mp_err mp_init_u32(mp_int *a, mp_u32 b) MP_WUR;

/* get integer, set integer and init with integer (mp_i64) */
mp_i64 mp_get_i64(const mp_int *a) MP_WUR;
void mp_set_i64(mp_int *a, mp_i64 b);
mp_err mp_init_i64(mp_int *a, mp_i64 b) MP_WUR;

/* get integer, set integer and init with integer, behaves like two complement for negative numbers (mp_u64) */
#define mp_get_u64(a) ((mp_u64)mp_get_i64(a))
void mp_set_u64(mp_int *a, mp_u64 b);
mp_err mp_init_u64(mp_int *a, mp_u64 b) MP_WUR;

/* get magnitude */
mp_u32 mp_get_mag_u32(const mp_int *a) MP_WUR;
mp_u64 mp_get_mag_u64(const mp_int *a) MP_WUR;
...
/* returns MP_YES if a can be reduced with mp_reduce_2k */
...
/* returns MP_YES if a can be reduced with mp_reduce_2k_l */
...
#if defined(MP_16BIT)
typedef mp_u32 mp_hval;
#else
typedef mp_u64 mp_hval;
#endif
...
mp_err mp_from_ubin(mp_int *a, const mp_u8 *buf, size_t size) MP_WUR;
mp_err mp_to_ubin(const mp_int *a, mp_u8 *buf, size_t maxlen, size_t *written) MP_WUR;

size_t mp_sbin_size(const mp_int *a) MP_WUR;
mp_err mp_from_sbin(mp_int *a, const mp_u8 *buf, size_t size) MP_WUR;
mp_err mp_to_sbin(const mp_int *a, mp_u8 *buf, size_t maxlen, size_t *written) MP_WUR;
