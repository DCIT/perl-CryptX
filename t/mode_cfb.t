...

{
...

  {
    ...
    is(Crypt::Mode::CFB->new('AES')->encrypt(undef, $key, $iv), '', 'encrypt(undef) returns empty string');
    is(Crypt::Mode::CFB->new('AES')->decrypt(undef, $key, $iv), '', 'decrypt(undef) returns empty string');
  }
}
