...
sub new {
  my ($class, @args) = @_;
  my $obj = Crypt::Cipher->new('XTEA', @args);
...
This module implements the XTEA cipher. Its interface is compatible with L<Crypt::CBC>.
...
C<new>, for example:
 ...
 my $c = Crypt::Cipher::XTEA->new($key);

=head2 new

 my $c = Crypt::Cipher::XTEA->new($key);
 #or
 my $c = Crypt::Cipher::XTEA->new($key, $rounds);
