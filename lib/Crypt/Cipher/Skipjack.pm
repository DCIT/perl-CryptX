...
sub new {
  my ($class, @args) = @_;
  my $obj = Crypt::Cipher->new('Skipjack', @args);
...
This module implements the Skipjack cipher. Its interface is compatible with L<Crypt::CBC>.
...
C<new>, for example:
 ...
 my $c = Crypt::Cipher::Skipjack->new($key);

=head2 new

 my $c = Crypt::Cipher::Skipjack->new($key);
 #or
 my $c = Crypt::Cipher::Skipjack->new($key, $rounds);
