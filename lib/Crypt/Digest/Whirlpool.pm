...
sub new {
  my ($class) = @_;
  my $obj = Crypt::Digest->new('Whirlpool');
...
=head1 SYNOPSIS
   ...
   my $data = 'data string';
   my $whirlpool_raw  = whirlpool($data);
   my $whirlpool_hex  = whirlpool_hex($data);
   my $whirlpool_b64  = whirlpool_b64($data);
   my $whirlpool_b64u = whirlpool_b64u($data);
   # or from file
   my $whirlpool_file_raw  = whirlpool_file('filename.dat');
   my $whirlpool_file_hex  = whirlpool_file_hex('filename.dat');
   my $whirlpool_file_b64  = whirlpool_file_b64('filename.dat');
   my $whirlpool_file_b64u = whirlpool_file_b64u('filename.dat');
   # or from filehandle
   my $filehandle = ...; # existing binary-mode filehandle
   my $whirlpool_fh_raw  = whirlpool_file($filehandle);
   my $whirlpool_fh_hex  = whirlpool_file_hex($filehandle);
   my $whirlpool_fh_b64  = whirlpool_file_b64($filehandle);
   my $whirlpool_fh_b64u = whirlpool_file_b64u($filehandle);
   ...
   my $file_result_raw = Crypt::Digest::Whirlpool->new->addfile('filename.dat')->digest;
...
Joins all arguments into a single string and returns its Whirlpool digest encoded as a binary string.
...
rules apply to C<whirlpool_hex>, C<whirlpool_b64>, and
C<whirlpool_b64u>.

 my $whirlpool_raw = whirlpool('data string');
 #or
 my $whirlpool_raw = whirlpool('any data', 'more data', 'even more data');

=head2 whirlpool_hex

Joins all arguments into a single string and returns its Whirlpool digest encoded as a hexadecimal string.

 my $whirlpool_hex = whirlpool_hex('data string');
 #or
 my $whirlpool_hex = whirlpool_hex('any data', 'more data', 'even more data');

=head2 whirlpool_b64

Joins all arguments into a single string and returns its Whirlpool digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $whirlpool_b64 = whirlpool_b64('data string');
 #or
 my $whirlpool_b64 = whirlpool_b64('any data', 'more data', 'even more data');

=head2 whirlpool_b64u

Joins all arguments into a single string and returns its Whirlpool digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $whirlpool_b64url = whirlpool_b64u('data string');
 #or
 my $whirlpool_b64url = whirlpool_b64u('any data', 'more data', 'even more data');

=head2 whirlpool_file

Reads a file given by a filename or filehandle and returns its Whirlpool digest encoded as a binary string.

 my $whirlpool_raw = whirlpool_file('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $whirlpool_raw = whirlpool_file($filehandle);

=head2 whirlpool_file_hex

Reads a file given by a filename or filehandle and returns its Whirlpool digest encoded as a hexadecimal string.

 my $whirlpool_hex = whirlpool_file_hex('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $whirlpool_hex = whirlpool_file_hex($filehandle);

B<Note:> The filehandle must be in binary mode before you pass it to C<addfile()>.

=head2 whirlpool_file_b64

Reads a file given by a filename or filehandle and returns its Whirlpool digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $whirlpool_b64 = whirlpool_file_b64('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $whirlpool_b64 = whirlpool_file_b64($filehandle);

=head2 whirlpool_file_b64u

Reads a file given by a filename or filehandle and returns its Whirlpool digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $whirlpool_b64url = whirlpool_file_b64u('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $whirlpool_b64url = whirlpool_file_b64u($filehandle);
