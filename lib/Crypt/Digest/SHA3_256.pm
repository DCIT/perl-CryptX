...
sub new {
  my ($class) = @_;
  my $obj = Crypt::Digest->new('SHA3_256');
...
Joins all arguments into a single string and returns its SHA3_256 digest encoded as a binary string.
...
rules apply to C<sha3_256_hex>, C<sha3_256_b64>, and
C<sha3_256_b64u>.
...
Joins all arguments into a single string and returns its SHA3_256 digest encoded as a hexadecimal string.
...
Joins all arguments into a single string and returns its SHA3_256 digest encoded as a Base64 string, B<with> trailing '=' padding.
...
Joins all arguments into a single string and returns its SHA3_256 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).
...
Reads a file given by a filename or filehandle and returns its SHA3_256 digest encoded as a binary string.
...
Reads a file given by a filename or filehandle and returns its SHA3_256 digest encoded as a hexadecimal string.
...
Reads a file given by a filename or filehandle and returns its SHA3_256 digest encoded as a Base64 string, B<with> trailing '=' padding.
...
Reads a file given by a filename or filehandle and returns its SHA3_256 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).
