...
sub new {
  my ($class) = @_;
  my $obj = Crypt::Digest->new('SHA3_384');
...
=head1 SYNOPSIS
   ...
   my $data = 'data string';
   my $sha3_384_raw  = sha3_384($data);
   my $sha3_384_hex  = sha3_384_hex($data);
   my $sha3_384_b64  = sha3_384_b64($data);
   my $sha3_384_b64u = sha3_384_b64u($data);
   # or from file
   my $sha3_384_file_raw  = sha3_384_file('filename.dat');
   my $sha3_384_file_hex  = sha3_384_file_hex('filename.dat');
   my $sha3_384_file_b64  = sha3_384_file_b64('filename.dat');
   my $sha3_384_file_b64u = sha3_384_file_b64u('filename.dat');
   # or from filehandle
   my $filehandle = ...; # existing binary-mode filehandle
   my $sha3_384_fh_raw  = sha3_384_file($filehandle);
   my $sha3_384_fh_hex  = sha3_384_file_hex($filehandle);
   my $sha3_384_fh_b64  = sha3_384_file_b64($filehandle);
   my $sha3_384_fh_b64u = sha3_384_file_b64u($filehandle);

   ### OO interface:
   use Crypt::Digest::SHA3_384;

   my $d = Crypt::Digest::SHA3_384->new;
   $d->add('any data');
   ...
   my $file_result_raw = Crypt::Digest::SHA3_384->new->addfile('filename.dat')->digest;
...
Joins all arguments into a single string and returns its SHA3_384 digest encoded as a binary string.
...
rules apply to C<sha3_384_hex>, C<sha3_384_b64>, and
C<sha3_384_b64u>.

 my $sha3_384_raw = sha3_384('data string');
 #or
 my $sha3_384_raw = sha3_384('any data', 'more data', 'even more data');

=head2 sha3_384_hex

Joins all arguments into a single string and returns its SHA3_384 digest encoded as a hexadecimal string.

 my $sha3_384_hex = sha3_384_hex('data string');
 #or
 my $sha3_384_hex = sha3_384_hex('any data', 'more data', 'even more data');

=head2 sha3_384_b64

Joins all arguments into a single string and returns its SHA3_384 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $sha3_384_b64 = sha3_384_b64('data string');
 #or
 my $sha3_384_b64 = sha3_384_b64('any data', 'more data', 'even more data');

=head2 sha3_384_b64u

Joins all arguments into a single string and returns its SHA3_384 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $sha3_384_b64url = sha3_384_b64u('data string');
 #or
 my $sha3_384_b64url = sha3_384_b64u('any data', 'more data', 'even more data');

=head2 sha3_384_file

Reads a file given by a filename or filehandle and returns its SHA3_384 digest encoded as a binary string.

 my $sha3_384_raw = sha3_384_file('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha3_384_raw = sha3_384_file($filehandle);

=head2 sha3_384_file_hex

Reads a file given by a filename or filehandle and returns its SHA3_384 digest encoded as a hexadecimal string.

 my $sha3_384_hex = sha3_384_file_hex('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha3_384_hex = sha3_384_file_hex($filehandle);

B<Note:> The filehandle must be in binary mode before you pass it to C<addfile()>.

=head2 sha3_384_file_b64

Reads a file given by a filename or filehandle and returns its SHA3_384 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $sha3_384_b64 = sha3_384_file_b64('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha3_384_b64 = sha3_384_file_b64($filehandle);

=head2 sha3_384_file_b64u

Reads a file given by a filename or filehandle and returns its SHA3_384 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $sha3_384_b64url = sha3_384_file_b64u('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha3_384_b64url = sha3_384_file_b64u($filehandle);
