...
=head1 SYNOPSIS
   ...
   my $data = 'data string';
   my $sha1_raw  = sha1($data);
   my $sha1_hex  = sha1_hex($data);
   my $sha1_b64  = sha1_b64($data);
   my $sha1_b64u = sha1_b64u($data);
   # or from file
   my $sha1_file_raw  = sha1_file('filename.dat');
   my $sha1_file_hex  = sha1_file_hex('filename.dat');
   my $sha1_file_b64  = sha1_file_b64('filename.dat');
   my $sha1_file_b64u = sha1_file_b64u('filename.dat');
   # or from filehandle
   my $filehandle = ...; # existing binary-mode filehandle
   my $sha1_fh_raw  = sha1_file($filehandle);
   my $sha1_fh_hex  = sha1_file_hex($filehandle);
   my $sha1_fh_b64  = sha1_file_b64($filehandle);
   my $sha1_fh_b64u = sha1_file_b64u($filehandle);
   ...
   my $file_result_raw = Crypt::Digest::SHA1->new->addfile('filename.dat')->digest;
...
Joins all arguments into a single string and returns its SHA1 digest encoded as a binary string.
...
rules apply to C<sha1_hex>, C<sha1_b64>, and
C<sha1_b64u>.

 my $sha1_raw = sha1('data string');
 #or
 my $sha1_raw = sha1('any data', 'more data', 'even more data');

=head2 sha1_hex

Joins all arguments into a single string and returns its SHA1 digest encoded as a hexadecimal string.

 my $sha1_hex = sha1_hex('data string');
 #or
 my $sha1_hex = sha1_hex('any data', 'more data', 'even more data');

=head2 sha1_b64

Joins all arguments into a single string and returns its SHA1 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $sha1_b64 = sha1_b64('data string');
 #or
 my $sha1_b64 = sha1_b64('any data', 'more data', 'even more data');

=head2 sha1_b64u

Joins all arguments into a single string and returns its SHA1 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $sha1_b64url = sha1_b64u('data string');
 #or
 my $sha1_b64url = sha1_b64u('any data', 'more data', 'even more data');

=head2 sha1_file

Reads a file given by a filename or filehandle and returns its SHA1 digest encoded as a binary string.

 my $sha1_raw = sha1_file('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha1_raw = sha1_file($filehandle);

=head2 sha1_file_hex

Reads a file given by a filename or filehandle and returns its SHA1 digest encoded as a hexadecimal string.

 my $sha1_hex = sha1_file_hex('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha1_hex = sha1_file_hex($filehandle);

B<Note:> The filehandle must be in binary mode before you pass it to C<addfile()>.

=head2 sha1_file_b64

Reads a file given by a filename or filehandle and returns its SHA1 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $sha1_b64 = sha1_file_b64('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha1_b64 = sha1_file_b64($filehandle);

=head2 sha1_file_b64u

Reads a file given by a filename or filehandle and returns its SHA1 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $sha1_b64url = sha1_file_b64u('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $sha1_b64url = sha1_file_b64u($filehandle);
