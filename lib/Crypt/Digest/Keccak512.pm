...
sub new {
  my ($class) = @_;
  my $obj = Crypt::Digest->new('Keccak512');
...
=head1 SYNOPSIS
   ...
   my $data = 'data string';
   my $keccak512_raw  = keccak512($data);
   my $keccak512_hex  = keccak512_hex($data);
   my $keccak512_b64  = keccak512_b64($data);
   my $keccak512_b64u = keccak512_b64u($data);
   # or from file
   my $keccak512_file_raw  = keccak512_file('filename.dat');
   my $keccak512_file_hex  = keccak512_file_hex('filename.dat');
   my $keccak512_file_b64  = keccak512_file_b64('filename.dat');
   my $keccak512_file_b64u = keccak512_file_b64u('filename.dat');
   # or from filehandle
   my $filehandle = ...; # existing binary-mode filehandle
   my $keccak512_fh_raw  = keccak512_file($filehandle);
   my $keccak512_fh_hex  = keccak512_file_hex($filehandle);
   my $keccak512_fh_b64  = keccak512_file_b64($filehandle);
   my $keccak512_fh_b64u = keccak512_file_b64u($filehandle);
   ...
   my $file_result_raw = Crypt::Digest::Keccak512->new->addfile('filename.dat')->digest;
...
Joins all arguments into a single string and returns its Keccak512 digest encoded as a binary string.
...
rules apply to C<keccak512_hex>, C<keccak512_b64>, and
C<keccak512_b64u>.

 my $keccak512_raw = keccak512('data string');
 #or
 my $keccak512_raw = keccak512('any data', 'more data', 'even more data');

=head2 keccak512_hex

Joins all arguments into a single string and returns its Keccak512 digest encoded as a hexadecimal string.

 my $keccak512_hex = keccak512_hex('data string');
 #or
 my $keccak512_hex = keccak512_hex('any data', 'more data', 'even more data');

=head2 keccak512_b64

Joins all arguments into a single string and returns its Keccak512 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $keccak512_b64 = keccak512_b64('data string');
 #or
 my $keccak512_b64 = keccak512_b64('any data', 'more data', 'even more data');

=head2 keccak512_b64u

Joins all arguments into a single string and returns its Keccak512 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $keccak512_b64url = keccak512_b64u('data string');
 #or
 my $keccak512_b64url = keccak512_b64u('any data', 'more data', 'even more data');

=head2 keccak512_file

Reads a file given by a filename or filehandle and returns its Keccak512 digest encoded as a binary string.

 my $keccak512_raw = keccak512_file('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $keccak512_raw = keccak512_file($filehandle);

=head2 keccak512_file_hex

Reads a file given by a filename or filehandle and returns its Keccak512 digest encoded as a hexadecimal string.

 my $keccak512_hex = keccak512_file_hex('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $keccak512_hex = keccak512_file_hex($filehandle);

B<Note:> The filehandle must be in binary mode before you pass it to C<addfile()>.

=head2 keccak512_file_b64

Reads a file given by a filename or filehandle and returns its Keccak512 digest encoded as a Base64 string, B<with> trailing '=' padding.

 my $keccak512_b64 = keccak512_file_b64('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $keccak512_b64 = keccak512_file_b64($filehandle);

=head2 keccak512_file_b64u

Reads a file given by a filename or filehandle and returns its Keccak512 digest encoded as a Base64 URL-safe string (see RFC 4648 section 5).

 my $keccak512_b64url = keccak512_file_b64u('filename.dat');
 #or
 my $filehandle = ...; # existing binary-mode filehandle
 my $keccak512_b64url = keccak512_file_b64u($filehandle);
