...
sub new {
  my ($class, @args) = @_;
  my $obj = Crypt::PRNG->new('Sober128', @args);
...
=head1 SYNOPSIS
   ...
   my $seeded_prng = Crypt::PRNG::Sober128->new("some data used for seeding PRNG");
...
functions, but use a hidden C<Crypt::PRNG::Sober128> object internally.
...
C<Crypt::PRNG::Sober128> object created via C<new>.

=head2 new

 my $prng = Crypt::PRNG::Sober128->new;
 my $seeded_prng = Crypt::PRNG::Sober128->new($seed);

Creates a PRNG object using the Sober128 algorithm. If C<$seed> is omitted,
