...
sub new {
  my ($class, @args) = @_;
  my $obj = Crypt::PRNG->new('Fortuna', @args);
...
functions, but use a hidden C<Crypt::PRNG::Fortuna> object internally.
...
C<Crypt::PRNG::Fortuna> object created via C<new>.

=head2 new

 my $prng = Crypt::PRNG::Fortuna->new;
 my $seeded_prng = Crypt::PRNG::Fortuna->new($seed);

Creates a PRNG object using the Fortuna algorithm. If C<$seed> is omitted, the
