...
=head1 SYNOPSIS
 ...
 my $signature = $alice->sign_message($message, 'SHA256');
 $alice_public->verify_message($signature, $message, 'SHA256') or die "ERROR";
...
B<BEWARE:> The C<$hash_name> default is C<'SHA1'> only for backward compatibility.
SHA-1 is vulnerable to practical collision attacks and is not safe for signing
messages whose content may be influenced by an attacker. Always pass an explicit
hash name such as C<'SHA256'>. The same applies to L</verify_message>.
...
Create signature (from commandline):
 ...
 openssl dgst -sha256 -sign eckey.priv.pem -out input.sha256-ec.sig input.data
...
Verify signature (Perl code):
 ...
 my $signature = read_rawfile("input.sha256-ec.sig");
 my $valid = $pkec->verify_hash($signature, digest_file("SHA256", "input.data"));
...
Create signature (Perl code):
 ...
 my $signature = $pkec->sign_hash(digest_file("SHA256", "input.data"));
 write_rawfile("input.sha256-ec.sig", $signature);

Verify signature (from commandline):

 openssl dgst -sha256 -verify eckey.pub.pem -signature input.sha256-ec.sig input.data
